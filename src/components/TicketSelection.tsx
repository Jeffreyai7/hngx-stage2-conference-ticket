...
const TicketSelection: React.FC<Props> = ({register, nextStep, errors, setValue }) => {
    ...
    <section className="w-[90%] mx-auto">
      <div className="bg-(--primaryColor) z-30 sticky top-[68px] mb-[32px]">
          <div className="flex justify-between w-[90%] mx-auto items-center  text-white">
            <h1 className="text-[24px] jeju-text md:text-[32px]">Ticket Selection</h1>
      ...
      <div>
        <div className="bg-custom-gradient my-[32px] w-[90%] mx-auto  text-white text-center px-6 py-4 md:p-6 border border-[#07373F] rounded-[24px]">
          <div>
            <h1 className="text-[2.5rem] md:text-[3.875rem] road-rage-text">Techember Fest "25</h1>
            <span className="text-[14px] roboto-text ">Join us for an unforgettable experience at [Event Name]! Secure your spot now.</span>
          </div>
          <div>
            <span className="text-[16px] roboto-text">📍 [Event Location] | | March 15, 2025 | 7:00 PM</span>
          </div>
        </div>
        ...
        <div>
            ...
            <div className="flex flex-col mt-2.5 gap-6 md:flex-row text-white">
              {
                ticketOptions.map((ticket) => {
                  return (
                    ...
                    className={`${selectedTicket === ticket.type ? "bg-(--selectColor)" : "hover:bg-(--hoverColor)"} flex-1 h-28 border border-[#197686] rounded-[12px] p-3 flex flex-col cursor-pointer hover:bg-(--hoverColor)`}>
                    <span className="text-[24px] font-semibold roboto-text" >{ticket.price}</span><span className="text-[16px] roboto-text">{ticket.label}</span><span className="text-[14px] roboto-text">20/52</span></div>
          ...
          <div className="flex flex-col w-[90%] gap-3 mt-6 pb-6 mx-auto md:flex-row">
            <Button className="jeju-text flex-1 border border-(--secondaryColor) text-(--secondaryColor) py-6 px-3 text-[16px] rounded-[12px] cursor-pointer">Cancel</Button>
            <Button className="jeju-text flex-1 border border-(--secondaryColor) text-white bg-(--secondaryColor) py-6 px-3 text-[16px] rounded-[12px] cursor-pointer" onClick={nextStep ? nextStep : undefined}>Next</Button>
          </div>
